...
def find_win_exe(program_name=r(i)chrome.exe$, default_exe_dirs=None)
    ...
    if default_exe_dirs is None
        default_exe_dirs = [CProgram Files, CProgram Files (x86),
                    DProgram Files, DProgram Files (x86), DUsers]
...
def find_chrome_util(exe_dirs=None) - str  None
    ...
    # Chromium 内核浏览器的启动配置，控制浏览器行为和初始状态：httpspeter.shexperimentschromium-command-line-switches
